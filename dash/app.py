...
Design : header dégradé, KPI cards colorées, navigation en haut
...
# ── Connexion MongoDB avec fix SSL pour Atlas ─────────────────────────────────
def charger():
    """Charge la collection indicateurs_propre depuis MongoDB.
    Compatible local et MongoDB Atlas (fix SSL)."""
    try:
        # Connexion Atlas (avec TLS)
        client = MongoClient(
            MONGO_URI,
            tls=True,
        ...
        # Fallback connexion locale sans TLS
        client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=30000)

    df = pd.DataFrame(
        list(client[DB_NAME]["indicateurs_propre"].find({}, {"_id": 0}))
    )
    client.close()
    return df
...
def calc_ratios(df):
    """Calcule les 5 ratios financiers principaux."""
    d = df.copy()
    def r(a, b):
    ...
    d["ratio_exploitation"] = np.round(r(d["charges_generales_exploitation"],
                                         d["produit_net_bancaire"]), 2)
...
# Coordonnées GPS des banques à Dakar
COORDS = {
...
# Palette de couleurs corporate sobre
PALETTE = [
...
#fbar{
  ...
  padding:12px 28px;display:flex;align-items:flex-end;gap:24px;
  box-shadow:0 1px 4px rgba(0,0,0,.04);flex-wrap:wrap;
}
...
.gc{
  ...
  padding:16px;height:100%;transition:box-shadow .15s;
}
...
.Select-option{color:#1e293b !important;font-size:12px !important}
...
.rc-slider-mark-text{color:#94a3b8 !important;font-size:10px !important}
...
def generer_pdf(sigle, annee):
    """Génère un rapport PDF pour une banque et une année données."""
    ...
    s_t = ParagraphStyle("t", parent=st["Title"], fontSize=18,
                         textColor=colors.HexColor("#0f172a"), spaceAfter=4)
    s_s = ParagraphStyle("s", parent=st["Normal"], fontSize=11,
                         textColor=colors.HexColor("#64748b"), spaceAfter=16)
    s_h = ParagraphStyle("h", parent=st["Heading2"], fontSize=12,
...
# CRITIQUE : server doit être défini AVANT app.run() pour Render/Gunicorn
server = app.server
...
# KPI cards : (id, label, couleur, icône)
KPI_CONF = [
...
def gc(title, gid, width=6, sub=""):
    """Crée une carte graphique avec titre et point bleu."""
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            html.Div([
                ...
                html.Div(
                    f"Analyse comparative · Base BCEAO · "
    ...
    html.Div([
        html.Div([
            ...
            dcc.Dropdown(
                ...
                value=int(ANNEE_DEF),
                clearable=False,
                style={"width":"110px","fontSize":"13px","fontWeight":"700"},
            ),
        ]),
        html.Div([
            ...
            dcc.Dropdown(id="fg",
                options=[{"label":"Tous","value":"tous"}] +
                        [{"label":g,"value":g} for g in GROUPES],
    ...
    html.Div([
        html.Div([
            html.Div(style={
                "height":"3px","background":acc,
                "borderRadius":"12px 12px 0 0",
                "margin":"-14px -16px 0",
            }),
            html.Span(ico, className="ki"),
            html.Div(lbl, className="kl"),
            html.Div(id=kid, className="kv", style={"color":acc}),
        ], className="kc")
        for kid,lbl,acc,ico in KPI_CONF
    ], id="kpi-row"),

    # Store pour l'onglet actif
    ...
    html.Div([
        ...
        html.Div([
            ...
            dbc.Row([gc("Radar — Comparaison multidimensionnelle","g18",12)],
                    className="g-3"),
        ], id="tab-t6", className="tab-pg"),

        # T7 — Tableau de données
        html.Div([
            html.Div([
                html.Div([html.Span(className="gcd"),
                          "Tous les indicateurs — filtrable & triable"],
                         className="gct"),
...
# Navigation sidebar → clientside (sans aller-retour serveur)
app.clientside_callback(
    ...
    function(at) {
        ...
        ['t1','t2','t3','t4','t5','t6','t7'].forEach(function(t){
            ...
            if(btn) btn.className = 'hn' + (t===at?' on':'');
        });
...
def switch_tab(*args):
    """Change l'onglet actif selon le bouton cliqué."""
...
def fil(annee, groupe, banques):
    """Filtre le DataFrame selon les sélections. Fallback si résultat vide."""
    d = df[df["annee"] == annee].copy()
    if groupe != "tous":
        d = d[d["groupe_bancaire"] == groupe]
    if banques:
...
def fmv(v):
    """Formate une valeur en milliards FCFA."""
...
# ── KPIs ──────────────────────────────────────────────────────────────────────
@app.callback(
    Output("k-b","children"), Output("k-e","children"),
    Output("k-r","children"), Output("k-f","children"),
    Output("k-roa","children"), Output("k-p","children"),
    Input("fa","value"), Input("fg","value"), Input("fb","value"),
)
def kpis(a, g, b):
    d = fil(a, g, b)
    if d.empty:
...
def tab_bilan(a, g, b):
    ...
    if d2.empty:
        ...
        f2 = go.Figure([
            go.Bar(name="Emplois", x=d2["sigle"], y=d2["emploi"],
                   marker_color="#2563eb", marker_opacity=0.85),
            go.Bar(name="Ressources", x=d2["sigle"], y=d2["ressources"],
                   marker_color="#0891b2", marker_opacity=0.85),
        ])
...
def tab_finance(a, g, b):
    d = fil(a, g, b)
    if d.empty:
        return efig(),efig(),efig()

    d1 = d.dropna(subset=["produit_net_bancaire"]).sort_values(
        "produit_net_bancaire", ascending=False)
    f1 = bl(px.bar(d1, x="sigle", y="produit_net_bancaire",
                   color="groupe_bancaire", color_discrete_sequence=PALETTE,
    ...
    if d2.empty:
        ...
        f2 = bl(px.bar(d2, x="sigle", y="resultat_net",
                       color="groupe_bancaire", color_discrete_sequence=PALETTE,
    ...
    if d3.empty:
        ...
        f3 = bl(px.bar(d3, x="sigle", y="ratio_exploitation",
                       color="groupe_bancaire", color_discrete_sequence=PALETTE,
...
def tab_ratios(a, g, b):
    ...
    f4.update_traces(textposition="top center",
...
@app.callback(
    ...
    Input("fg","value"), Input("fb","value"),
)
def tab_evol(g, b):
    d = df.copy()
    if g != "tous":
        d = d[d["groupe_bancaire"] == g]
    if b:
        d = d[d["sigle"].isin(b)]
    if d.empty:
        return efig(),efig(),efig()
    ...
    return (ln("bilan","Bilan (M FCFA)"),
            ln("resultat_net","Résultat net"),
            ln("ratio_roa","ROA (%)"))

# ── Tab 5 — Carte Sénégal ─────────────────────────────────────────────────────
@app.callback(
    Output("g15","figure"),
    Input("fa","value"), Input("fc","value")
)
def carte(a, indic):
    rows = []
    ...
    fig = px.scatter_mapbox(
        dfc, lat="lat", lon="lon", text="sigle",
...
def tab_comp(a, g, b):
    d = fil(a, g, b)
    if d.empty:
        return efig(320),efig(320),efig(340)
    ...
    d2 = d.dropna(subset=["resultat_net"]).sort_values(
        "resultat_net", ascending=False).head(10)
    if d2.empty:
        f2 = efig(320)
    else:
    ...
    cols_r = ["ratio_roa","ratio_roe","ratio_solvabilite",
              "ratio_exploitation","ratio_marge_nette"]
    d3 = d.dropna(subset=cols_r)
    if d3.empty:
        return f1, f2, efig(340)
    ...
    for i,(_,row) in enumerate(dn.head(6).iterrows()):
        ...
        f3.add_trace(go.Scatterpolar(
            r=vs, theta=labs+[labs[0]], name=row["sigle"],
...
def tableau(a, g, b):
    ...
    return (d.to_dict("records"),
            [{"name":c.replace("_"," ").upper(),"id":c} for c in ok])
